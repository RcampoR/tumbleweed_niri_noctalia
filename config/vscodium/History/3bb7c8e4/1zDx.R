...
# ==========================
# Abundancia por especie y ambiente
# ==========================

# Abundancia por especie × ambiente
abundancia_ambiente <- BD_original %>%
  group_by(especie, ambiente) %>%
  summarise(abundancia = sum(abundancia), .groups = "drop")

# Orden descendente por abundancia TOTAL
orden_especies <- BD_original %>%
  group_by(especie) %>%
  summarise(total = sum(abundancia), .groups = "drop") %>%
  arrange(desc(total))

abundancia_ambiente <- abundancia_ambiente %>%
  mutate(especie = factor(especie, levels = orden_especies$especie))

# ==========================
# Gráfico apilado
# ==========================

grafica_abundancia <- ggplot(
  abundancia_ambiente,
  aes(x = especie, y = abundancia, fill = ambiente)
) +
  geom_col(width = 0.7) +
  geom_text(
    data = orden_especies,
    aes(x = especie, y = total, label = total),
    inherit.aes = FALSE,
    vjust = -0.5,
    size = 3.5,
    colour = "grey30"
  ...
  scale_fill_manual(
    values = c("Lago" = "#6DB388", "rio" = "#6E9FD4"),
    labels = c("Lago", "Río"),
    name = "Ambiente"
  ) +
  scale_y_continuous(
...
ggsave(
  filename = here("figuras", "abundancia_especies_guartinajas.png"),
  plot = grafica_abundancia,
  width = 11,
  height = 7,
  dpi = 1000
)
